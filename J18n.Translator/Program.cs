...
public class J18nJoint : ICloneable
{
    ...
    /// </summary>
    private HashSet<string>? childrenKeys => this.Children?.Select(c => c._key).ToHashSet();
...
#pragma warning restore IDE1006 // 命名样式
    ...
    private static void UpdateModificationTime(J18nJoint? joint)
    {
        StringBuilder path = new();
        J18nJoint? current = joint;
    ...
    private void SetParent(IEnumerable<J18nJoint> children , CancellationToken? ctsToken = null)
    {
        ...
        var result = Parallel.ForEach(children , parallelOptions , child => child.Parent = this);
    ...
    {
        return UpdateChild((c) => c._key.Equals(newChild._key) , newChild , FullUpdateAction , out oldJoint);
    }

    public bool UpdateChild(int index , J18nJoint newChild , out J18nJoint? oldJoint)
    {
        return UpdateChild((c) => c.Index.Equals(index) , newChild , FullUpdateAction , out oldJoint);
    }

    public bool UpdateChild(string key , J18nJoint newChild , out J18nJoint? oldJoint)
    {
        return UpdateChild((c) => c._key.Equals(key , StringComparison.Ordinal) , newChild , FullUpdateAction , out oldJoint);
    }
    ...
    public bool UpdateChild(string key , string rawText , J18nJointType type , out J18nJoint? oldJoint , string comment = "" , string description = "")
    {
        ...
        return UpdateChild((c) => c._key.Equals(key , StringComparison.Ordinal) , newChild , PartialUpdateAction , out oldJoint);
    ...
    public bool UpdateChild(int index , string rawText , J18nJointType type , out J18nJoint? oldJoint , string comment = "" , string description = "")
    {
        ...
        return UpdateChild((c) => c.Index.Equals(index) , newChild , PartialUpdateAction , out oldJoint);
    }
    ...
    public bool UpdateChild(Func<J18nJoint , bool> findOldChild , J18nJoint newChild , Action<J18nJoint , J18nJoint> updateAction , out J18nJoint? oldJoint)
    {
        ...
        if(oldChild is not null)
        {
            ...
            UpdateModificationTime(oldChild);
            return true;
        }
        oldJoint = null;
        return false;
    ...
    public bool RemoveChild(Func<J18nJoint , bool> findTheChild , out J18nJoint? removedJoint)
    {
    ...
    public async Task<(bool, J18nJoint?)> RemoveChildAsync(Func<J18nJoint , bool> findTheChild , CancellationToken ctsToken)
    {
        Children ??= new HashSet<J18nJoint>();
    ...
    {
        return RemoveChild((c) => c._key.Equals(key , StringComparison.Ordinal) , out removedJoint);
    }

    public bool RemoveChild(int index , out J18nJoint? removedJoint)
    {
        return RemoveChild((c) => c.Index == index , out removedJoint);
    }
...
public class J18nNode
{
    private JObject _node;
    private ITranslationOption TranslationOption;
    public bool IsLeaf => !_node.Children().Any();
    public JTokenType Type => _node.Type;
    public string? Key => _node.Path;
    public string? RawText => _node.ToString(formatting: Formatting.Indented);


    public J18nNode(JObject node)
    {
        _node = node;
    }

    public void UpdateValue(JToken newToken , JToken oldToken)
    {
        _node.ReplaceAll(newToken);
        _node.DeepClone();
    }
